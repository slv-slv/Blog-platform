...
export const extractPagingParams = (req: Request, res: Response, next: NextFunction) => {
  const sortBy = req.query.sortBy ?? SETTINGS.PAGING_DEFAULT_PARAMS.sortBy;
  const sortDirection = req.query.sortDirection ?? SETTINGS.PAGING_DEFAULT_PARAMS.sortDirection;
